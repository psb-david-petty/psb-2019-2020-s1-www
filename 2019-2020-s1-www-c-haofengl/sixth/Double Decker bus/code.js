...
function setWatchColorPicker(setSelector, changeSelector) {
    ...
    function watchColorPicker(event) {
        document.querySelectorAll(changeSelector).forEach(function(s) {
            s.style.backgroundColor = event.target.value;
            /* https://stackoverflow.com/a/9792527 */
            rgb = s.style.backgroundColor.match(/[0-9]+(?=[,)])/g);
            console.log('background:', rgb);
            let r = parseInt(rgb[0]), g = parseInt(rgb[1]), b = parseInt(rgb[2]);
            /* https://www.geeksforgeeks.org/how-to-change-text-color-depending-on-background-color-using-javascript/ */
            let yiq = Math.round((r * 299 + g * 587 + b * 114) / 1000);
            console.log('YIQ:', yiq);
            s.style.color = `rgb(${255 - r}, ${255 - g}, ${255 - b})`;
            s.style.color = yiq > 125 ? 'black' : 'white';
            console.log('foreground:', s.style.color);
        });
    }

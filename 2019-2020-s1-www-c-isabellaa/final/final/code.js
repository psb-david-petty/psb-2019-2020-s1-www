function showAnswers(form) {
   ...
   */
  const valid = new Set(
    document.getElementById('valid').getAttribute('data-valid').split(' ')
  ...
  let inputs /* all input nodes */ = document.getElementsByTagName('input');
   ...
   */
  let figures /* all figure nodes */ = document.getElementsByTagName('figure');
  ...
  for (let entry of map.entries()) {
    let key = entry[0],
      value = entry[1];
    document.getElementById(key).innerHTML = '' + key + ' = ' + map.get(key) + '; ';
